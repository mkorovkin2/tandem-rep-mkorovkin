...
def simulation_set():
    ...
    hg_set = hg_set.loc[hg_set["array_length"] < 5000]
...
def run_linear_SVM(X_train, X_test, y_train, y_test, X_total, y_total, alpha=0.01, C=0.1, d=0.01, d_penalty=1.0, bound_mod=1.0, lr=0.001, fp_penalty=1., epochs=100000):
    batch_size = 96 # np.max(len(X_train) // 100, 96)

    x_data = tf.placeholder(shape=(None, 2), dtype=tf.float32)
    y_label = tf.placeholder(shape=(None, 1), dtype=tf.float32)

    A = tf.Variable(tf.divide(tf.random_normal(shape=[2, 1]), 10))
    b = tf.Variable(tf.divide(tf.random_normal(shape=[1, 1]), 10))
    ...
    error_residuals = tf.reduce_sum(
        tf.subtract(
            tf.multiply(1., bound_mod), tf.add(
    ...
    fp_result = tf.divide(tf.reduce_sum(
        tf.cast(
            tf.equal(
                tf.not_equal(
                    tf.equal(y_label, 1),
                    tf.equal(prediction, 1)
                ),
                tf.equal(prediction, 1)
            ), tf.float32)
    ),
    ...
    optimizer = tf.train.AdamOptimizer(lr)# AdamOptimizer(lr)# 0.01)# 0.001) # tf.train.GradientDescentOptimizer(0.00000001)
    ...
    for i in range(epochs):
        ...
        false_positive_train = sess.run(fp_result, feed_dict={
            x_data: X_test,
            y_label: y_test
        })
        ...
        if (i + 1) % 300 == 0:
            if False:
                ...
                print('Step #{} | False positive rate = {}'.format(str(i + 1), false_positive_train))
